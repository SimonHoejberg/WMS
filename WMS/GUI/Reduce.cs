        ...
        private void reduceConfirmBtn_Click(object sender, EventArgs e)
        {
            ...
            if (a.Equals(DialogResult.OK))
            {
                ...
                for (int i = 0; i < reduceDataGridView.RowCount; i++)
                {
                    ...
                    {
                        core.DataHandler.ReduceItem('-',reduceDataGridView[0, i].Value.ToString(), reduceDataGridView[1, i].Value.ToString(), int.Parse(reduceDataGridView[6,i].Value.ToString()), user,LogOperations.REDUCED);
                    }
                }
            }
        }
