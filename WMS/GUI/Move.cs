...
namespace WMS.GUI
{
    public partial class Move : Form, IGui
    {
        ...
        private void ConfirmButtonClick(object sender, EventArgs e)
        {
            ...
            {
                if (dgvRow.Index == dataGridView.Rows.Count - 1)//We don't want the last and empty row
                {
                    break;
                }
            ...
            if (noProblemsEncountered == true) //Commit changes if no problems.
            {
                ...
                if (a.Equals(DialogResult.OK))
                {
                    ...
                    for (int i = 0; i < rowCount; i++)
                    {
                        ...
                        {
                            core.DataHandler.ItemMove(tempOldLoc.Id.ToString(), (tempOldLoc.Quantity - Convert.ToInt32(dataGridView.Rows[i].Cells[quantityColumnString].Value)).ToString(), tempOldLoc.ItemNo.ToString());
                            core.DataHandler.ItemMove(tempNewLoc.Id.ToString(), (Convert.ToInt32(dataGridView.Rows[i].Cells[quantityColumnString].Value) + tempNewLoc.Quantity).ToString(), tempOldLoc.ItemNo.ToString());
                            core.DataHandler.ItemMove(tempNewLoc.ItemNo, tempNewLoc.LocationString);
                            core.DataHandler.MoveActionOnItem(tempOldLoc.ItemNo, core.DataHandler.GetItemFromItemNo(tempOldLoc.ItemNo).Description, core.GetTimeStamp(), Convert.ToInt32(dataGridView.Rows[i].Cells["QuantityColumn"].Value), core.UserName, $"{tempOldLoc.LocationString} -> {tempNewLoc.LocationString}");
                        }
                        else
                        {
                            core.DataHandler.ItemMove(tempOldLoc.Id.ToString(), (tempOldLoc.Quantity - Convert.ToInt32(dataGridView.Rows[i].Cells[quantityColumnString].Value)).ToString(), tempOldLoc.ItemNo.ToString());
                            core.DataHandler.ItemMove(tempNewLoc.Id.ToString(), Convert.ToInt32(dataGridView.Rows[i].Cells[quantityColumnString].Value).ToString(), tempOldLoc.ItemNo.ToString());
                            core.DataHandler.ItemMove(tempNewLoc.ItemNo, tempNewLoc.LocationString);
                            core.DataHandler.MoveActionOnItem(tempOldLoc.ItemNo, core.DataHandler.GetItemFromItemNo(tempOldLoc.ItemNo).Description, core.GetTimeStamp(), Convert.ToInt32(dataGridView.Rows[i].Cells["QuantityColumn"].Value), core.UserName, $"{tempOldLoc.LocationString} -> {tempNewLoc.LocationString}");
                        }
